...
# ╔═╡ c5d9cfe7-9c99-42b5-b389-e03deb7cd440
using RDatasets, DataFrames, CSV

# ╔═╡ d6dac2f8-0c1b-4418-9845-78eb5d6fa183
using MLJ, Random, Plots, MLJLinearModels, MLJDecisionTreeInterface

# ╔═╡ 9ee7c3cd-c967-4546-bbef-431df3881b43
using HypothesisTests

# ╔═╡ 213c8ca1-ff1c-4136-9f23-11e350607f3c
using PyPlot

# ╔═╡ aa56c0fe-e64e-4b63-a7a0-1f953ed5ed00
md"""# INFO411 Assignment 2 - Josh Signal - Modelling"""

# ╔═╡ 8aa12ab8-6214-4713-a11f-13772b20cef2
begin
	DS1 = CSV.read("statlog-heart.data.txt", DataFrame,header= vec(["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal", "num"]))
	DS1[:,:num] = [item > 1.5 ? 0 : 1 for item in DS1[:,:num]]
	DS2 = CSV.read("DS2_imputed.csv", DataFrame,header=true)
	DS2_bin = copy(DS2)
	DS2_bin[:,:num] = [item > 0.5 ? 1 : item for item in DS2_bin[:,:num]]
end;

# ╔═╡ 0b6ce9b8-cdf9-45c3-b27e-3b9e2a7ca475
begin
	#data = coerce(boston, autotype(boston, :discrete_to_continuous))
	y = DS2.num
	X = select(DS2, Not(:num))
end;

# ╔═╡ 402f7c3e-e335-4e67-b1c6-289281049035
md"""
## Regression on DS2
"""

# ╔═╡ 8a6f9cc0-f232-4537-b712-c4ddd985c5ce
md""" ### Decision Tree Regressor"""

# ╔═╡ bcf67584-c6be-4b6e-8631-4338cceb5c81
DecisionTreeRegressor = @load DecisionTreeRegressor pkg=DecisionTree verbosity=0

# ╔═╡ 3b85f3a8-a4c9-4817-8cff-f416ae453f6a
md"""
Start with a forest of 200 trees, each using 5 random subfeatures: 
"""

# ╔═╡ c92d08b9-6558-47ee-8e3c-98ef9b10d265
begin
	tree_reg = DecisionTreeRegressor()
	forest = EnsembleModel(model=tree_reg, n=200)
	forest.model.n_subfeatures = 5
end

# ╔═╡ d6f78655-ef52-41d1-8209-f77f329d28d9
md"""
(Hyper)parameters of the ensemble model: 
"""

# ╔═╡ b4ad8b10-a2db-4f57-b34e-34c541c8aed9
params(forest) 

# ╔═╡ 8af41b89-7dc7-48f6-92c7-49ff1c9fbb9c
md""" 
Cross validation to evaluate our model: """

# ╔═╡ 98be63ca-13b8-4183-8ee0-2a25dc8fb9bc
begin
	Random.seed!(1)
...
# ╔═╡ 94867709-d7f6-4fec-b569-ca308d22f5a8
md"""
## Comparing with Ridge Regressor
"""

# ╔═╡ 2f5d0891-9797-4561-8893-a115a1bdd1d4
RidgeRegressor = @load RidgeRegressor pkg=MLJLinearModels verbosity=0

# ╔═╡ 38d190da-ad02-4b2b-8557-d9e3a4936c2b
# Task 1
begin
	Random.seed!(1)
	reg = RidgeRegressor()
	machrr = machine(reg, X, y)
	rr_perf = evaluate!(machrr, resampling=CV(nfolds=10), measure=rms, verbosity=0)
end

# ╔═╡ 33920ccf-b28f-4e43-b22b-e28975bb45c7
begin
	using StatsPlots
	violin(["Ridge" "RF"], [rr_perf.per_fold[1], perf.per_fold[1]], legend=false)
end

# ╔═╡ 490cb7c1-9498-4841-ab82-9dfeb05e9e5a
MannWhitneyUTest(rr_perf.per_fold[1], perf.per_fold[1])

# ╔═╡ 0a03f4ed-d54f-4e58-8212-a09016d41284
println(
	"The Ridge Regressor mean is ",mean(rr_perf.per_fold[1]), 
	"\n and standard deviation is: ", std(rr_perf.per_fold[1]), 
	"\nThe Random Forest mean is ", mean(perf.per_fold[1]), 
	"\n and standard deviation is: ",std(perf.per_fold[1])
)

# ╔═╡ 678e64ea-bbca-4c4e-a84c-9756c4ff90b9
md""" The average RMS measures are not statistically different.
"""

# ╔═╡ a9dd98bc-fce4-443f-bbb7-c9c22dd1a91f
md"""
### Model tuning experiment
"""

# ╔═╡ d3bcc5e9-87b7-4879-9dc2-bf66e13c9614
md"""
Vary number of trees and generate a learning curve for RMS:
"""

# ╔═╡ 9213d274-7cf6-4c10-bc2b-62c3c620589a
begin
	r_n = range(forest, :n, lower=50, upper=500)
	curve = learning_curve(mach, resampling=CV(nfolds=10),
	                         range=r_n, measure=rms)
	Plots.plot(curve.parameter_values, curve.measurements, ylabel="RMS", xlabel="# of trees", label="")
end

# ╔═╡ 4a9c5ff9-b7f3-4675-9a2c-20aac02aeeab
md"""
Learning curve by varying the maximal depth from 1 to 15:
"""

# ╔═╡ c8f7680c-397a-4564-a41d-a93872903ffa
begin
	r_bf = range(forest, :(model.max_depth), lower=1, upper=15)
	curve_bf = learning_curve(mach, resampling=CV(nfolds=10),
	                         range=r_bf, measure=rms);
	Plots.plot(curve_bf.parameter_values, curve_bf.measurements, ylabel="RMS", xlabel="max_depth", label="")
end

# ╔═╡ 9b9446f9-45d5-452a-b104-aa5e3d0b4836
md"""
### Automatic tuning - grid search


#### Grid search for n\_subfeatures and bagging\_fraction:
"""

# ╔═╡ e155b301-e324-4700-8e45-a6b555ba93a9
begin
	Random.seed!(1)
...
# ╔═╡ 45ba3dc3-d8b0-4fd5-ab43-312162fda0b8
md"""
Best performance obtained: 
"""

# ╔═╡ 26c2340d-cfc3-4c9d-b8a8-80cdcf610685
begin
	rep = report(m)
	rep.best_history_entry.measurement[1]
end

# ╔═╡ 2cacab68-1f39-4625-a007-88f08c2a0855
md"""best\_history\_entry: """

# ╔═╡ 91c4348a-c30d-4a16-af3c-03601ac343f4
rep.best_history_entry

# ╔═╡ 372520ea-686d-4966-9423-aebc4040c023
begin
	res = rep.plotting
...
# ╔═╡ 0b51973f-a48f-485a-8a40-a28ebb300b4a
begin
	plotly()
	pl=Plots.contour(vals_sf, vals_bf, res.measurements, cmap=:hot, fill=true, size=(780,460))
	xlabel!("# of sub-features", fontsize=14)
	ylabel!("Bagging fraction", fontsize=14)
	pl
end

# ╔═╡ e3cbe228-22da-48ce-9f79-9862c434f94a
md"""
#### Grid search n\_subfeatures and max\_depth:
"""

# ╔═╡ aa7891df-d39d-482c-82f5-9e5fca16f6e0
begin
	Random.seed!(1)
	r31 = range(forest, :(model.n_subfeatures), lower=1, upper=6)
	r32 = range(forest, :(model.max_depth), lower=2, upper=15)
	tm3 = TunedModel(model=forest, tuning=Grid(resolution=10),
	                resampling=CV(nfolds=6), ranges=[r31, r32],
	                measure=rms)
	m3 = machine(tm3, X, y)
	MLJ.fit!(m3);
	rep3 = report(m3)
...
# ╔═╡ d5a9feb2-1273-4b5b-bb61-5fd53b7cbed1
rep3.best_history_entry

# ╔═╡ 9a14f120-eb5b-4c94-a54f-1aa49f706f3f
(rep3.best_history_entry.measurement[1], rep3.best_history_entry.model.model.max_depth, rep3.best_history_entry.model.model.n_subfeatures)

# ╔═╡ f26cf100-854c-43dd-87b4-7bbc263351df
md"""
- Best seting: n\_subfeatures=3, max\_depth=6.
- RMS=0.932375. 
"""

# ╔═╡ 4b45d528-d521-4d07-83f3-83835109ca56
begin
	res3 = rep3.plotting
	vals_nsf = res3.parameter_values[:, 1]
	vals_md = res3.parameter_values[:, 2];
	p3 = figure(figsize=(10,6))
	tricontourf(vals_nsf, vals_md, res3.measurements, cmap=get_cmap("hot"))
	xlabel("Number of sub-features", fontsize=14)
	ylabel("Max. depth", fontsize=14)
	colorbar(label="RMS")
	p3
end

# ╔═╡ e565afb0-b11b-4f86-8f11-ca8153b52dfa
md"""
On average off by about 1 on a 0-4 scale (25%). Not great performance but not horrible. The data is quite noisy and clunkily put together.
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "a47d3c8004349e1c9fe4a34c4c2a0f4ab9a20358"
